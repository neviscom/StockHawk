...
import com.sam_chordas.android.stockhawk.ui.MyToast;
...
public class StockTaskService extends GcmTaskService {
    ...
    String fetchData(@NonNull String url) throws IOException {
    ...
    public int onRunTask(TaskParams params) {
        ...
        return doRequest(urlStringBuilder.toString());
    }

    private void init(@NonNull StringBuilder urlStringBuilder) {
    ...
    private int doRequest(@NonNull String urlString) {
        ...
        try {
            ...
            if (isUpdate) {
                ...
                getContentResolver().update(QuoteProvider.Quotes.CONTENT_URI, contentValues, null, null);
            }
            ArrayList<ContentProviderOperation> operations = JsonParserUtils.quoteJsonToContentVals(response);
            if (!operations.isEmpty()) {
                getContentResolver().applyBatch(QuoteProvider.AUTHORITY, operations);
            } else {
                handleEmptyResponse();
            }
    ...
    private void handleError() {
        if (NetworkUtils.isConnected(this)) {
            showMessage(R.string.common_error);
        } else {
            showMessage(R.string.network_toast);
        }
    }
    ...
    private void showMessage(@StringRes final int stringId) {
        new Handler(Looper.getMainLooper()).post(new Runnable() {
            @Override
            public void run() {
                MyToast.showToast(StockTaskService.this, stringId);
            }
        });
    }
